...
TEST(ClockReplacerTest, Victim) {
  auto lru_replacer = new ClockReplacer(1010);

  // Empty and try removing
  int result;
  EXPECT_EQ(0, lru_replacer->Victim(&result)) << "Check your return value behavior for ClockReplacer::Victim";

  // Unpin one and remove
  lru_replacer->Unpin(11);
  EXPECT_EQ(1, lru_replacer->Victim(&result)) << "Check your return value behavior for ClockReplacer::Victim";
  EXPECT_EQ(11, result);
...
TEST(ClockReplacerTest, Size) {
  auto lru_replacer = new ClockReplacer(10010);
...
TEST(ClockReplacerTest, ConcurrencyTest) {
  ...
  for (int run = 0; run < num_runs; run++) {
    int value_size = 1000;
    std::shared_ptr<ClockReplacer> lru_replacer{new ClockReplacer(value_size)};
...
TEST(ClockReplacerTest, IntegratedTest) {
  int result;
  int value_size = 10000;
  auto lru_replacer = new ClockReplacer(value_size);
  std::vector<int> value(value_size);
  for (int i = 0; i < value_size; i++) {
    value[i] = i;
  }
